export const metadata = {
  title: 'Train On Your Groupchat',
  subtitle: 'Using LoRA and in-browser inference to fine-tune your friends',
  date: '2025-11-07',
};

> You can jump in here: [infinitegroupchat.com ↗](https://groupme-web.vercel.app).  
> It’s powered by a fine-tuned LLM running on-device (iOS26 / WebGPU required)
...
I knew up front that I wanted to fine-tune instead of using RAG. This was partly for the learning experience, but also because it felt like fine-tuning would be more akin to the original Markov chain approach than a RAG-based one would’ve been.
...
Actual training was straightforward. Since the notebook was already set up in Google Colab, I could just hit “run” on each cell and wait. I was able to use a free T4 GPU instance for about 6 hours, periodically pausing and running some inference to test. I saw it learning...
...
...until at about 80% completion, when a “runtime disconnected” message popped up. I clicked around, momentarily oblivious to the fact that **I lost everything.**
...
The references here (“deece”, “retreat”, “joss”) are semi-inscrutable outside our chat, but the model had learned specific place vocabulary, as well as our penchant for logistics. 3.5 years of chat history were now encoded into one 250MB LoRA adapter, or roughly 2.5GB when merged back into the original model.
...
Not wanting to drain my credits further, I ported the Python script from running on Google’s cloud GPUs to running locally on my Macbook’s CPU by replacing Colab’s inference with [Ollama](https://github.com/ollama/ollama-python). This required downloading the merged model (LoRA adapter + original) as a single, 2.5GB [GGUF](https://huggingface.co/docs/hub/en/gguf) file locally.
...
We use vLLM for inference at work, but that felt like too much to manage for a hobby project. I also really didn’t want to pay for a beefy GPU - I had already seen how well the model ran on a 2018 Macbook CPU.
...
*[Wherein the top answer is just “Server”](https://www.reddit.com/r/LocalLLaMA/comments/1kh4joc/where_are_you_hosting_your_fine_tuned_model/)*
...
The worst part was that after waiting the 6 minutes, the machine would wake up for 1 second, see no requests, and then immediately scale back to zero, by design. I could have remedied this by foregoing the serverless approach entirely, and keeping the instance always on for roughly $20 a month. I didn’t feel ready to compromise.

## We’re gonna need a smaller model

I banged my head for a while. I resolved that we’d need a smaller-than-2.5GB model to improve cold starts. But how small? I considered buying a Raspberry Pi to self-host the model for a one-time cost.
...
The last step was to convert the .safetensors format into WebLLM’s MLC format so that we could run it on the web. I found MLC’s [conversion process](https://llm.mlc.ai/docs/compilation/convert_weights.html) fiddly and prone to segfault, possibly due to my Intel Mac not being well supported. After failing to convert locally, I set up a dedicated [MLC Conversion notebook](https://colab.research.google.com/drive/1NbdAzcVNGsCSHNTFnOMYpmp-pDwntukw?usp=sharing) on Colab’s GPUs, and yolo-hardcoded the version to cu-124 nightly. That worked.

The resulting model quality was notably worse than the raw LoRA, likely due to aggressive quantization (q4f16_1). Still, it was passable, and we’d dropped the merged model size by **10x** - from roughly 2.5GB for the merged 4B param model, down to 250MB for quantized 0.6B.
...
Other than the “language model in the browser” part, [the final web app](https://groupme-web.vercel.app/) is dead simple. It started as a single HTML file with vanilla JS, and only at the very end did I port it to Vite and React. The app is client-side rendered and served with static assets on Vercel. The model weights themselves (~250MB) are also static assets, being served from HuggingFace’s CDN at initial load, and subsequently from a browser cache.

I realized late on that I could even run a model this small in my phone’s browser: I just needed to upgrade my iPhone 14 Pro to iOS26 for [WebGPU support](https://caniuse.com/webgpu). I saw frequent crashes with Qwen3 0.6B that seemed to imply serious memory pressure, so I did one more training run with Qwen2.5 0.5B. This was just enough to keep the tab using less than a 1GB of memory on iOS, a threshold I found on this [Github issue](https://github.com/mlc-ai/web-llm/issues/386#issuecomment-2207631803). This ended up being the default model (“Mobile”) that I shipped, but the whole family ([Groupchat 4B](https://huggingface.co/brimtown/Groupchat-Qwen3-4B-MLC), [Groupchat 0.6B](https://huggingface.co/brimtown/Groupchat-Qwen3-0.6B-MLC), and [Groupchat 0.5B](https://huggingface.co/brimtown/Groupchat-Qwen2.5-0.5B-MLC)) are selectable in the app.

It’s not all roses. The app still crashes on phones due to memory pressure, so we persist the conversation in browser localStorage to allow quick resumption after refresh. This tiny, fried model also not infrequently outputs Chinese characters (Qwen is produced by Alibaba), so we filter characters in that Unicode range out of responses. I’m tickled that the original 4B param model is available on the web, for true sickos willing to wait for a 2.5GB download.


On the plus side, generation speed is not an issue with the smallest model; I even added a toggleable delay feature to slow things down. Lastly, I added a “Group Topic” feature that is just injected into the prompt, instructing the target speaker to “talk ONLY about [GroupTopic]”.
...
Some of these decisions are similar to Claude Code’s architecture, like storing all conversations on-device (in our case, in localStorage), and the lack of a traditional “backend” (just calls to a Chat Completions-style inference API). For the latter, the difference is that ours runs entirely locally. We’ve essentially transformed compute into storage, by replacing a model served behind an HTTP API with a model’s weights being served from a CDN.

For the purposes of an (admittedly insane) side project, I’m happy with the tradeoffs. Platforms like Vercel, HuggingFace, and GitHub give out free storage, and as a developer I’m more than ok to let users ‘bring their own compute’ without my credit card going brrrrr.

## Future work

Having trained my first (three) models, I would love to revisit the fine-tuning process. I’d move away from my ‘no intervention’ data approach and do a bit more curation of the 50,000 examples. In the current models, in addition to stripping out URLs, I had to pull tricks like raising the temperature high just to get everyone to stop talking about what time lunch was.
